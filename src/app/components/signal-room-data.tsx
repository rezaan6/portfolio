...
const messages = await import("../locales/" + locale + ".json")`,
  ...
  },
  {
    ...
    decision:
      `I architected and delivered the CMS from 0→1 to MVP as React domain modules with SOLID boundaries, then made two investments that most teams defer. First, a Storybook-based reusable component library — the shared spine, so a new screen is composed rather than authored. Second, Test-Driven Development with Cypress, taken seriously enough to reach ${MEASUREMENTS.coverage.value} automated coverage and wired into CI/CD, so a release is gated by evidence rather than by whoever remembered to click through it. Redux handled state flow, TanStack React Query handled fetching, caching, and synchronization, and Node.js BFF layers cut API latency.`,
    tradeoff:
...
→ read the plan before blaming the render layer.`,
     ...
     skills group and the project tech chips, so a keyword screen still sees it. */
  {
    ...
    howIUse:
      "Vitest is what I set the Hobber test layer up on, from an empty repository. It shares the application's resolve and transform config, so there is no second build setup drifting out of sync with the first — one alias map, one plugin list. What it does not do is prove the production bundle: tests run through esbuild in test mode, while the release build adds tree-shaking, minification and chunking on top. That gap is real, and it is why I read bundle size on every diff rather than trust a green suite to catch it — which is a habit, not a gate, and the honest word for it matters: nothing in the pipeline stops a merge on size today. Below the unit tests I use it for integration tests at the slice boundaries, because in a feature-sliced codebase the contract between slices is exactly where a regression will hide.",
